...
def active_scan_output():
    try:
        ...
        output='Failed'
    output_frame = CTkScrollableFrame(master=tabview.tab("ACTIVE SCAN"), width=1000, height=500)
...
def pas_scan_res():
    try:
        ...
        output='Failed'
    output_frame = CTkScrollableFrame(master=tabview.tab("PASSIVE SCAN"), width=1000, height=500)
    output_frame.pack(padx=10,pady=10)
    o=CTkLabel(master=output_frame,text=output)
    o.pack()
    return o
...
#         except BaseException:
#             output='Failed'
#         output_frame = CTkScrollableFrame(master=tabview.tab("HISTORY SCAN"), width=1000, height=500)
#         output_frame.pack(padx=10,pady=10)
#         o=CTkLabel(master=output_frame,text=output)
#         o.pack()
#         return o
...
#History Scan Tab
username=CTkEntry(master=tabview.tab("HISTORY SCAN"),placeholder_text="Enter username of you computer",width=900,height=40,font=("Arial",18))
username.pack(pady=10)
# hist_act_btn=CTkButton(master=tabview.tab("HISTORY SCAN"),text="Start Active Scan",height=50,width=100,font=("Arial Black",18),command=history_act_scan)
# hist_act_btn.pack()
hist_pas_btn=CTkButton(master=tabview.tab("HISTORY SCAN"),text="Start Passive Scan",height=50,width=100,font=("Arial Black",18),command=history_pas_scan)
hist_pas_btn.pack()
# warning=CTkLabel(master=tabview.tab("HISTORY SCAN"),text='Please Scan only active or passive at a time\n Wait for the Output')
# warning.pack()
